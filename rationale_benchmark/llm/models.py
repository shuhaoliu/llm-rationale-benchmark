...
class ProviderConfig:
  ...
  def __post_init__(self):
    ...
    if self.timeout <= 0:
      raise ConfigurationError(f"Timeout must be positive for provider '{self.name}'")
    
    if self.max_retries < 0:
      raise ConfigurationError(f"Max retries cannot be negative for provider '{self.name}'")
...
class LLMConfig:
  ...
  @classmethod
  def from_dict(cls, config_dict: Dict[str, Any], config_file: str = "") -> "LLMConfig":
...
class ModelRequest:
  ...
  def __post_init__(self):
    ...
    if self.stop_sequences is not None and not isinstance(self.stop_sequences, list):
